...
const Dashboard = () => {
    ...
    const handleUserClick = (user) => {
        console.log("Handling task click:", user);

        if (!user.id) {
            console.error("No ID found in task", user);
            return;
        }
    ...
    return (
        <div className="dashboard-container">
            <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <Card className="card-custom col-span-1">
                        <CardContent>
                            ...
                            <Typography variant="body1"
                                        className="text-muted">Assigned: {dashboardData.taskCount}</Typography>
                            <Typography variant="body1" className="text-muted">In
                                Progress: {dashboardData.inProgressCount}</Typography>
                            <Typography variant="body1"
                                        className="text-muted">Pending: {dashboardData.pendingCount}</Typography>
                            <Typography variant="body1"
                                        className="text-muted">Completed: {dashboardData.completedCount}</Typography>
                        </CardContent>
                    </Card>
                )}
                ...
                {(userRole === "ADMIN" || userRole === "DEPARTMENT_HEAD") && (
                    <Card className="card-custom col-span-1 md:col-span-2 lg:col-span-1">
                        <CardContent>
                            ...
                            <Typography variant="body1"
                                        className="text-muted">Total: {dashboardData.taskCount}</Typography>
                            <Typography variant="body1"
                                        className="text-muted">Pending: {dashboardData.pendingCount}</Typography>
                            <Typography variant="body1" className="text-muted">In
                                Progress: {dashboardData.inProgressCount}</Typography>
                ...
                {/* Загрузка сотрудников для главы отдела */}
                ...
                {/* Данные только для главы отдела */}
                {userRole === "DEPARTMENT_HEAD" && (
                    <Card className="card-custom col-span-1">
                        <CardContent>
                            <Typography variant="h6" className="section-header">Department Overview</Typography>
                            <Typography variant="body1" className="text-muted">Active
                                Employees: {dashboardData.activeUserCount}</Typography>
                        </CardContent>
                    </Card>
                )}
                    ...
                    <>
                        <Card className="card-custom col-span-1">
                            <CardContent>
                                ...
                                <Typography variant="body1" className="text-muted">Total
                                    Users: {dashboardData.userCount}</Typography>
                            </CardContent>
                        </Card>
                                        ...
                                        dashboardData.inactiveUsers.map(emp => (
                                            ...
                                            >
                                                ...
                                                <ListItemText primary={emp.username}
                                                              secondary={`Last login: ${emp.lastLogin} days ago`}/>
                                            </ListItem>
                                        ))
                                    ) : (
                                        <Typography variant="body2" className="text-muted">No inactive
                                            users</Typography>
                                    )}
                                </List>
                            </CardContent>
